...
exports.login = async (req, res, next) => {
    ...
    try {
      ...
      if (currentUser.passwordChangedAt) {
        ...
        if (passChangedTimestamp > decoded.iat) {
          throw new Error('User recently changed his password. Please log in again.');
        }
      }
  ...
  exports.hasRoles = (...roles) => async (req, res, next) => {
    try {
  ...
  exports.resetPassword = async (req, res, next) => {
    try {
      const user = await User.findOne({ email: req.body.email });
      if (!user) {
        return res.status(404).json({ message: `There is no user with email ${req.body.email}` });
      }
